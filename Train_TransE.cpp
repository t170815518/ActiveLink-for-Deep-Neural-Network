             		...
             		for (int k=0; k<batchsize; k++)
             		{
						...
						if (rand()%1000<pr)
						{
							while (ok[make_pair(fb_h[i],fb_r[i])].count(j)>0){
                                ...
                                if (ok.count(make_pair(fb_h[i],fb_r[i]))==0)
                                    break;
							}
...
void prepare()
{
    FILE* f1 = fopen("data/FB15k-237/entity2id.txt","r");
	FILE* f2 = fopen("data/FB15k-237/relation2id.txt","r");
	int x;
    ...
    FILE* f_kb = fopen("data/FB15k-237/train.txt","r");
